...
class auto_vk_toolkit_starter_app : public avk::invokee
{
...
public: // v== avk::invokee overrides which will be invoked by the framework ==v
  auto_vk_toolkit_starter_app(avk::queue& aQueue)
    : mQueue{ &aQueue }
  ...
  void update() override
  {
    ...
    if (++counter == 4) {
      ...
      printf("Time from init to fourth frame: %ld min, %ld sec %lf ms\n", int_min, int_sec - static_cast<decltype(int_sec)>(int_min) * 60, fp_ms - 1000.0 * static_cast<double>(int_sec));
    }
...
int main() // <== Starting point ==
{
  ...
  try {
    ...
    auto app = auto_vk_toolkit_starter_app(singleQueue);
    // Create another element for drawing the UI with ImGui
    auto ui = avk::imgui_manager(singleQueue);

    // Compile all the configuration parameters and the invokees into a "composition":
    auto composition = configure_and_compose(
      avk::application_name("Auto-Vk-Toolkit Starter"),
